...
def load_svs_bruker(fid_filename, acqp_filename=None, method_filename=None):
    ...
    ACQP = bapi.jcampdx.JCAMPDX.read_jcampdx(acqp_filename)
    ...
    METHOD = bapi.jcampdx.JCAMPDX.read_jcampdx(method_filename)
    
    try:
        dt = float(METHOD['PVM_DigDw'].array[0])*1e-3*2
    except:
        try:
          dt = float(METHOD['PVM_SpecDwellTime'].array[0])*1e-6*2
        except:
            raise FileNotFoundError("parameter not found")
    
    try:
        digitiser_delay = int(ACQP['ACQ_RxFilterInfo'].array[0][0])
    except:
        try:
           digitiser_delay = int(METHOD['PVM_DigShift'].array[0])
        except:
            raise FileNotFoundError("parameter not found") 
    
    try:
        f0 = float(ACQP['BF1'].array[0])
    except:
        raise FileNotFoundError("parameter not found") 

...
class Dataset:
    ...
    **Example:**
    ...
    def load_schema(self):
        ...
        if self.type == 'fid':
            if self.subtype == 'refscan':
                self._schema = SchemaRefScan(self)
            else:
                self._schema = SchemaFid(self)
        elif self.type == '2dseq':
            self._schema = Schema2dseq(self)
        elif self.type == 'rawdata':
            if self.subtype == 'job0':
                self._schema = SchemaRawdata(self)
            elif self.subtype == 'job1':
                self._schema = SchemaNavdata(self)
            elif self.subtype == 'Navigator':
                self._schema = SchemaNavdata(self)
        elif self.type == 'ser':
    ...
    def _read_data(self):
        ...
        if self.subtype == 'refscan':
            ...
            data = self._read_binary_file(self.path, self.numpy_dtype, self.schema.layouts['shape_storage'])

...
class SchemaRefScan(Schema):
    @property
    def layouts(self):
        layouts={}
        layouts['shape_storage'] = (2, int(self._dataset.block_size/2), self._dataset.channels)
        return layouts
    
    def deserialize(self, data, layouts):
        return np.squeeze(data[0::2,...] + 1j * data[1::2,...])
...
class SchemaRawdata(Schema):
    ...
    def deserialize(self, data, layouts):
        return np.squeeze(data[0::2,...] + 1j * data[1::2,...],0)
...
class SchemaNavdata(Schema):

    @property
    def layouts(self):
        layouts={}
        layouts['raw']=(int(int(self._dataset.parameters['acqp']['ACQ_jobs'].array[1][0])/2), self._dataset.channels , int(self._dataset.job_desc[3]))
        layouts['shape_storage'] = (2, int(int(self._dataset.parameters['acqp']['ACQ_jobs'].array[1][0])/2), self._dataset.channels , int(self._dataset.job_desc[
                                                                                                  3]))
        layouts['final'] = layouts['raw']
        return layouts
